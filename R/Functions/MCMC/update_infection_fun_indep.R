## update_infection_fun_indep.m ################################################
update_infection_fun_indep <- function(theta, data_struct_augmented_old, ll_household_old, ll_household_form, t_i_prop_sd_symp) {
  # Update the infection times of symptomatic infected hosts in the
  ...
  # Randomly select one symptomatic infected host in every household containing at least one such individual
  r1                        <- runif(sum(symp_in_household))
  update_hosts_in_household <- ceiling(no_symp_in_household[symp_in_household] * r1)
  household_start_indices   <- cumsum(household_sizes_incl) - household_sizes_incl
  update_hosts              <- household_start_indices[symp_in_household] + update_hosts_in_household
  
  # Update the infection times of the chosen hosts
  r2          <- rnorm(sum(symp_in_household))
  t_i_updates <- t_i_prop_sd_symp * r2
  ...
  t_s_dir_prop   <- t_s_old[sort_indices]
  ...
  # Calculate the log-likelihood contribution from each household using the proposed infection times
  ...
  acceptance <- list(
    ...
    overall   = mean(accept_households[symp_in_household]),
    asymp     = NaN
  )
  acceptance$symp <- acceptance$overall
