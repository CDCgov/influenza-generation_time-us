## update_asymp_fun_mech.m #####################################################
update_asymp_fun_mech <- function(theta, data_struct_augmented_old, ll_household_old, ll_household_form, t_prop_sd_asymp) {
  ...
  t_i_prop[update_hosts] <- t_i_prop[update_hosts] + t_updates
  t_s_prop[update_hosts] <- t_s_prop[update_hosts] + t_updates
  ...
  t_s_dir_prop   <- t_s_prop[sort_indices]
  symp_dir_prop  <- symp[sort_indices]
  asymp_dir_prop <- asymp[sort_indices]
  
  # Populate the structure array data_struct_augmented_prop with the proposed infection and onset times
  ...
  acceptance <- list(
    ...
    overall   = mean(accept_households[asymp_in_household & (no_infected_in_household > 1)]),
    infection = NaN
  )
