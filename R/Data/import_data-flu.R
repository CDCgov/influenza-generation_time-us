...
import_data <- function(i, param_seq, scn) {
  ...
  df_dat <- readRDS('../R/Data/fludat_deid.RDS')
  
  # Removing unused columns
  df_dat$cdc_num_flu_valid          <- NULL
  ...
  df_dat$cdc_site                   <- NULL
  df_dat$cdc_flu_docordate_doses    <- NULL
  df_dat$vaccine_flu_doses          <- NULL
  df_dat$x_vaccination              <- NULL
  df_dat$cdc_flu_pos_lab            <- NULL
  df_dat$cdc_flua_pos_lab           <- NULL
  df_dat$cdc_flub_pos_lab           <- NULL
  df_dat$cdc_hhflutype              <- NULL
  ...
  # One season
  if(period == "2021_22") df_dat <- df_dat %>% filter(cdc_season == "2021-22")
  if(period == "2022_23") df_dat <- df_dat %>% filter(cdc_season == "2022-23")
  # Removing column
  df_dat$cdc_season                 <- NULL
  
  # Type A or B
  if(type == "A") df_dat <- df_dat %>% filter(corr_hhflutype %in% c("A_notsubtyped", "A_H3", "A_H1", "Coinfection_AH1AH3"))
  if(type == "B") df_dat <- df_dat %>% filter(corr_hhflutype %in% c("B"))
  # Removing column
  df_dat$corr_hhflutype                 <- NULL
  
  # No co-primary cases
  if(coprimary == FALSE) df_dat <- df_dat %>% filter(cdc_flu_coprimary == 0)
  # Removing column
  df_dat$cdc_flu_coprimary          <- NULL
  ...
  # Logical vectors
  df_dat$cdc_flu_pos       <- (df_dat$cdc_flu_pos       == 1) # tested positive
  df_dat$cdc_ever_symptoms <- (df_dat$cdc_ever_symptoms == 1) # symptomatic (not necessarily infected)
  
  df_dat$symp_all  <- ( df_dat$cdc_flu_pos) & ( df_dat$cdc_ever_symptoms) #  symptomatic infected
  df_dat$asymp_all <- ( df_dat$cdc_flu_pos) & (!df_dat$cdc_ever_symptoms) # asymptomatic infected
  df_dat$uninf_all <- (!df_dat$cdc_flu_pos)                               #            uninfected
  ...
  # Find index cases
  df_dat$cdc_flu_primary <- (df_dat$cdc_flu_primary == 1)
  ...
  df_dat$d_iR_all[df_dat$cdc_flu_primary] <- pmin(df_dat$d_iR_all[df_dat$cdc_flu_primary], 0)  # since date where index swabbed positive taken as time zero
  
  # Right bounds for date of infection
  # using the dates of positive test
  df_dat$d_iR_all[df_dat$cdc_flu_pos] <- pmin(df_dat$d_iR_all[df_dat$cdc_flu_pos], df_dat$cdc_flu_firstanyposdt[df_dat$cdc_flu_pos])  # must have been infected before positive test
  ...
  for (household in unique(df_dat$cdc_hhid)) {
    ...
    if ((no_in_household > 1) && (is.na(household_onset_diff_max) || household_onset_diff_max <= max_onsetdiff)) {
      ...
      # print discarded individual id
      cat("Discarded individual ID:", household_inds, "\n")
    }
  }
